...
#ifdef DEBUG_SIGNAL
	lock();
		cout<<endl<<"!!!SIGNAL: NIT "<<((PCB*)PCB::running)->getID()<<" JE NASILNO UGASENA";
	unlock();
#endif
	PCB::running->listWaiting->notifyAll();
	delete PCB::running->myEvent;	//	OSLOBADJANJE DOGADJAJA
	if (PCB::running->sem!=0)	//	OSLOBADJANJE SEMAFORA
...
#ifdef DEBUG_THREAD
	lock();
		cout<<endl<<"NIT "<<((PCB*)PCB::running)->getID()<<" JE ZAVRSILA";
	unlock();
#endif
	PCB::running->listWaiting->notifyAll();
	PCB::running->status = 4;
	if (PCB::running->owner!=0 && PCB::running->owner!=PCB::kernelPCB)	// SINGAL 1 - SLANJE KREATORU DA JE NIT ZAVRSILA
		PCB::running->owner->myThread->signal(1);
...
#ifdef DEBUG_SIGNAL
	lock();
		cout<<endl<<"SIGNAL: NIT "<<((PCB*)PCB::running)->getID()<<" OBRADJUJE SIGNALE ";
		PCB::running->signalsArrived->ispis();
	unlock();
#endif
	PCB::running->signalsArrived->signalRoutine();
	lock();
...
#ifdef DEBUG_SIGNAL
	lock();
		cout<<endl<<"SIGNAL: NIT "<<((PCB*)PCB::running)->getID()<<" JE ZAVRSILA OBRADU SIGNALA ";
		PCB::running->signalsArrived->ispis();
	unlock();
#endif
	unlock();
