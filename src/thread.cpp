...
#ifdef DEBUG_SIGNAL
	lock();
		cout<<endl<<"SIGNAL "<<signal<<" JE POSLAT NITI "<<myPCB->getID()<<" OD NITI "<<((PCB*)PCB::running)->getID();
	unlock();
#endif
		myPCB->signalsArrived->addSignal(signal);
		//	ODBLOKIRANJE NITI AKO JE CEKALA NA SIGNAL
		if ((myPCB->signalStates[signal]==0 && PCB::globalSignalStates[signal]==0) && myPCB->status==6){
			myPCB->status = 1;
			Scheduler::put(myPCB);
		}
		//	UMETANJE RUTINE ZA OBRADU SIGNALA AKO JE VEC IZVRSENA
...
void Thread::unblockSignal(SignalId signal){
	lock();
	myPCB->signalStates[signal] = 0;
	myPCB->signalStateChanged=1;	// 	PROMENJEN STATUS, KRENI RUTINU ZA OBRADU SIGNALA ISPOCETKA
